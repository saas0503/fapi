...
func GenerateToken(userid string, ttl time.Duration, privateKey string) (*TokenDetails, error) {
	now := time.Now().UTC()
	td := &TokenDetails{
		ExpiresIn: new(int64),
		Token:     new(string),
...
func VerifyToken(token string, publicKey string) (*TokenDetails, error) {
	...
	return &TokenDetails{
		TokenUuid: fmt.Sprint(claims["id"]),
